...
******************************************************************************/
HI_S32 SAMPLE_COMM_VENC_SaveStream(FILE* pFd, VENC_STREAM_S* pstStream,vio_para *para)
{
    HI_S32 i;
    ...
    for (i = 0; i < pstStream->u32PackCount; i++)
    {
        ...
        if(para->cb!=NULL){
            para->cb(0,pstStream->pstPack[i].pu8Addr + pstStream->pstPack[i].u32Offset,pstStream->pstPack[i].u32Len - pstStream->pstPack[i].u32Offset,para->user_arg);
		}
    }
        ...
        else
        {
            if (FD_ISSET(s32VencFd, &read_fds))
            {
                ...
                if(bSaveJpg || bSaveThm)
                {
                    ...
                    s32Ret = SAMPLE_COMM_VENC_SaveStream(pFile, &stStream,NULL);
                    if (HI_SUCCESS != s32Ret)
                    {
...
static void sample_yuv_8bit_dump(VIDEO_FRAME_S* pVBuf, vio_para* para,FILE *pfd)
{
    unsigned int w, h;
    char* pVBufVirt_Y;
    char* pVBufVirt_C;
...
#define MaxBufNum 8
HI_VOID* SAMPLE_COMM_SendFrameProc(HI_VOID* p)
{
    HI_U32 i,j,VeChnCnt;
    HI_S32 s32Ret;
    VIDEO_FRAME_INFO_S  *pstVideoFrame;
    USER_FRAME_INFO_S   stFrame[MaxBufNum];
    ...
    pfd = fopen("vpss_1280x720.yuv", "wb");
    if(pfd == NULL)
    {
    ...
    while(HI_TRUE == pstPara->bThreadStart)
    {
        ...
        if(pstPara->para.cb!=NULL)
        {
            static int num = 3;
            if(num--)
                sample_yuv_8bit_dump( &pstVideoFrame->stVFrame, &pstPara->para,pfd);

        }
        //SAMPLE_PRT("HI_MPI_VPSS_GetChnFrame width=%d height=%d\n",pstVideoFrame->stVFrame.u32Width,pstVideoFrame->stVFrame.u32Height);
        for(VeChnCnt=0; VeChnCnt<pstPara->s32Cnt; VeChnCnt++)
        {
            ...
            s32Ret = HI_MPI_VENC_SendFrame(pstPara->VeChn[VeChnCnt], &stFrame[i],-1);
            if(HI_SUCCESS != s32Ret)
            {
...
HI_S32 SAMPLE_COMM_VENC_SendFrame(VPSS_GRP VpssGrp,VPSS_CHN VpssChn,VENC_CHN VeChn[],HI_S32 s32Cnt,SIZE_S stSize,vio_para* para)
{
    HI_S32 i;
    ...
    stQpMapSendFramePara.para = *para;
    for(i=0; i<s32Cnt;i++)
    {
        stQpMapSendFramePara.VeChn[i] = VeChn[i];
    }

    return pthread_create(&gs_VencQpmapPid, 0, SAMPLE_COMM_SendFrameProc, (HI_VOID*)&stQpMapSendFramePara);
}
...
//HI_S32 SAMPLE_COMM_VENC_SendFrame(VPSS_GRP VpssGrp,VPSS_CHN VpssChn,VENC_CHN VeChn[],HI_S32 s32Cnt,SIZE_S stSize)
//{
//    HI_S32 i;
...
HI_VOID* SAMPLE_COMM_VENC_GetVencStreamProc(HI_VOID* p)
{
    ...
    ******************************************/
    ...
    for (i = 0; i < s32ChnTotal; i++)
    {
        ...
        if(PT_JPEG != enPayLoadType[i] && pstPara->para.debug)
        {
...
#ifndef __HuaweiLite__
                    s32Ret = SAMPLE_COMM_VENC_SaveStream(pFile[i], &stStream, &pstPara->para);
#else
                    ...
                    for (s32Cnt = 0; s32Cnt < 3; s32Cnt++)
                    {
                        switch (s32Cnt)
                        {
                            case 0:
                                ...
                                {
                                    #ifndef __HuaweiLite__
                                    s32Ret = SAMPLE_COMM_VENC_SaveStream(pFile[i+s32Cnt], &stStream,NULL);
                                    #else
...
******************************************************************************/
HI_S32 SAMPLE_COMM_VENC_StartGetStream(VENC_CHN VeChn[],HI_S32 s32Cnt,vio_para* para)
{
    HI_U32 i;
